...
from datasets.loaders import get_train_loader, get_val_loaders, get_test_loaders, get_mol_test_loaders, get_mol_val_loaders
from unsupervised.embedding_evaluation import GeneralEmbeddingEvaluation
from unsupervised.encoder import Encoder, FeaturedEncoder
from unsupervised.learning import GInfoMinMax
from unsupervised.utils import initialize_edge_weight
from unsupervised.view_learner import ViewLearner
from utils import setup_wandb, get_total_mol_onehot_dims, wandb_cfg_to_actual_cfg
...
def run(args):
    ...
    # Force data to be only molpcba for training, with all features
    ...
    dataloader = get_train_loader(args.batch_size, my_transforms, subset=["chemical", "dummy"],
                                   social_excludes=excludes, for_adgcl = True)

    val_loaders, names = get_mol_val_loaders(args.batch_size, my_transforms)
    ...
    if not random_dropping:
        ...
        view_feature_model = FeaturedEncoder(
    ...
    # Evaluate at epoch 0
    general_ee.embedding_evaluation(model.encoder, val_loaders, test_loaders,
                                     names, node_features = evaluation_node_features)
    ...
    for epoch in tqdm(range(1, args.epochs + 1)):
        ...
        if epoch % 25 == 0:
            ...
            checkpoint_name = f"{wandb.run.dir}/Epoch-{epoch}{'-random_dropping' if random_dropping else ''}-with-features.pt"
...
def arg_parse():
    ...
    parser.add_argument('--checkpoint', type=str, default="untrained", help='Either the name of the trained model checkpoint in ./outputs/, or latest for the most recent trained model in ./wandb/latest-run/files')
